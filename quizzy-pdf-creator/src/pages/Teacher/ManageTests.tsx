...
const ManageTests: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'active' | 'assigned' | 'stopped'>('all');
    ...
    if (query) {
      filtered = filtered.filter(
        test => 
          ...
          test.pdf_name.toLowerCase().includes(query)
      );
    }
  ...
  const handleTabChange = (tab: 'all' | 'active' | 'assigned' | 'stopped') => {
    setActiveTab(tab);
