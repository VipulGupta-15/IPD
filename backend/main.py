...
def generate_mcq_with_relevance(text, groq_api_key, num_questions=2, difficulty="medium"):
    ...
    try:
        ...
        prompt = (
            ...
            f"Each question is a JSON object with: question (string), options (array of 4 strings), "
...
def assign_test():
    ...
    tests_collection.update_one(
        {"user_id": user_id, "test_name": test_name},
        {"$set": {"assigned_to": valid_student_ids_str, "start_time": start_time, "end_time": end_time, "duration": duration, "status": "assigned"}}
    )
...
def manage_test():
    ...
    if action == "start":
        ...
        tests_collection.update_one(
            {"user_id": user_id, "test_name": test_name},
            {"$set": {"assigned_to": valid_student_ids_str, "start_time": start_time, "end_time": end_time, "status": "assigned"}}
        )
...
def export_results():
    ...
    writer.writerow(['Student ID', 'Score', 'Total', 'Details'])
    for student_id, result in results.items():
        details = "; ".join([f"Q{detail['mcq_index']}: {detail['is_correct']}" for detail in result['details']])
        writer.writerow([student_id, result['score'], result['total'], details])
