...
def select_text_from_pdf(pdf_path, batch_size=1000, num_batches=2):
    pdf_text = extract_text_from_pdf(pdf_path)
    total_length = len(pdf_text)
    selected_text = ""
    for _ in range(num_batches):
        start_index = random.randint(0, total_length - batch_size)
        selected_text += pdf_text[start_index:start_index + batch_size]
    return selected_text
